...
const SGP_URL = process.env.SGP_URL || "https://foxfibrasp.sgp.net.br";
...
const CONTACT_WHATSAPP = clean(process.env.CONTACT_WHATSAPP || "", 40);
const CONTACT_EMAIL = clean(process.env.CONTACT_EMAIL || "", 180);
const SITE_URL = clean(process.env.SITE_URL || "", 220);
...
const PUBLIC_HOST = (() => {
  try {
    return (PUBLIC_BASE_URL ? new URL(PUBLIC_BASE_URL).hostname : "cadastro.foxfibra.com.br").toLowerCase();
  } catch {
    return "cadastro.foxfibra.com.br";
  }
})();
const ALLOW_RENDER_HOST = String(process.env.ALLOW_RENDER_HOST || "false") === "true";
const KEEPALIVE_URL = clean(process.env.KEEPALIVE_URL || `${PUBLIC_BASE_URL || "https://cadastro.foxfibra.com.br"}/health`, 300);
...
const contractConfig = {
  ...
  formacobrancaId: Number(process.env.CONTRACT_FORMACOBRANCA_ID || process.env.CONTRACT_FORMA_COBRANCA_CODIGO || 0),
  formaCobrancaCodigo: Number(process.env.CONTRACT_FORMA_COBRANCA_CODIGO || 0),
...
async function geocodeAddress(address) {
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        "Accept": "application/json",
        "User-Agent": "FoxFibraCadastro/1.0 contato@foxfibra.com.br"
      },
...
async function handleContractTerm(req, res, url) {
  ...
  try {
    ...
    send(res, 200, termHtml, {
      ...
      "Cache-Control": "no-store",
      "Content-Security-Policy": "default-src 'self' https://foxfibrasp.sgp.net.br; img-src 'self' data: blob: https:; style-src 'self' 'unsafe-inline' https://foxfibrasp.sgp.net.br; script-src 'self' 'unsafe-inline' https://foxfibrasp.sgp.net.br; connect-src 'self' https://foxfibrasp.sgp.net.br; form-action 'self' https://foxfibrasp.sgp.net.br; base-uri 'self'; frame-ancestors 'none'"
    });
    return true;
  } catch (error) {
  ...
  const ping = async () => {
    try {
      const response = await fetch(KEEPALIVE_URL, {
        headers: { "User-Agent": "FoxFibraCadastro-KeepAlive/1.0" },
        signal: AbortSignal.timeout(12000)
      });
